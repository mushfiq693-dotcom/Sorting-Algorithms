...
export function BugHunt({ algorithmId }: BugHuntProps) {
  ...
  const handleVerifyFix = (e: React.FormEvent) => {
    ...
    if (selectedLine !== challenge.buggyLineNumber) {
      ...
      setFeedback({
        status: "wrong-line",
        message: `Line ${selectedLine} is actually correct! Check loop boundaries or comparison conditions instead. (-10 pts penalty)`,
      });
      return;
    }
    ...
    if (!isFixValid) {
      ...
      setFeedback({
        status: "wrong-fix",
        message: `You identified the right line (Line ${selectedLine})! But this edit still doesn't produce an ascending sort. (-10 pts penalty)`,
      });
      return;
    }
    ...
    <div className="flex flex-col gap-5 rounded-2xl border border-border bg-card p-4 sm:p-6 backdrop-blur-md shadow-sm dark:shadow-2xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-5 flex flex-col gap-4">
          ...
          <div className="rounded-2xl border border-border bg-card p-3.5 backdrop-blur-sm shadow-sm">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                disabled={isRunning}
                className="inline-flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-blue-600 to-cyan-600 px-2.5 py-1 text-xs font-semibold text-white hover:from-blue-500 transition-all disabled:opacity-40 cursor-pointer"
              >
