...
export function BanglaNote({ topic, banglaText, defaultExpanded = false }: BanglaNoteProps) {
  ...
  return (
    <div className="my-6 rounded-2xl border border-emerald-500/30 bg-emerald-950/20 backdrop-blur-md shadow-lg overflow-hidden transition-all duration-200 hover:border-emerald-500/50">
      {/* Header Toggle Button */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        aria-expanded={isOpen}
        className="w-full flex items-center justify-between px-5 py-4 text-left transition-colors bg-emerald-500/10 hover:bg-emerald-500/15"
      >
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="p-5 sm:p-6 border-t border-emerald-500/20 text-sm text-emerald-100/90 leading-relaxed font-sans space-y-3 select-text">
              <div className="prose prose-invert prose-emerald max-w-none text-xs sm:text-sm whitespace-pre-line leading-relaxed">
                {banglaText.trim()}
              </div>
