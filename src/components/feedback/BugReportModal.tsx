...
export function BugReportModal({ isOpen, onClose, algorithmId }: BugReportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
      <div
        className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto touch-scroll rounded-2xl border border-border bg-card p-5 sm:p-7 text-foreground shadow-2xl backdrop-blur-2xl space-y-4 animate-in zoom-in-95"
      ...
      >
        ...
        <div className="space-y-1">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-rose-500/30 bg-rose-500/10 text-rose-700 dark:text-rose-300 font-mono text-[11px] font-semibold">
        ...
        {isSuccess ? (
          <div className="py-10 text-center space-y-3 animate-in fade-in">
            ...
            <h3 className="text-base font-bold text-foreground">Bug Report Logged!</h3>
            <p className="text-xs text-muted-foreground">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 font-sans">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full rounded-xl border border-border bg-background px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground focus:border-rose-500 focus:outline-none transition-colors"
              />
            </div>
            ...
            <div className="space-y-1">
              <label className="block text-xs font-semibold text-foreground">
                Steps to Reproduce <span className="text-rose-500">*</span>
              </label>
              <textarea
                required
                rows={2}
                ...
                className="w-full rounded-xl border border-border bg-background p-2.5 text-xs text-foreground placeholder:text-muted-foreground focus:border-rose-500 focus:outline-none transition-colors resize-none"
              />
            </div>
            ...
            {/* Error Alert */}
            {errorMsg && (
              <div className="p-2.5 rounded-xl border border-rose-500/50 bg-rose-500/15 text-rose-700 dark:text-rose-200 text-xs flex items-start gap-2 animate-in fade-in">
