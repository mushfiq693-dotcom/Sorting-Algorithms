...
export function OperationIndicator({
  ...
  return (
    <div className={`flex flex-col gap-3 rounded-2xl border bg-[#0b101d]/90 p-4 sm:p-5 backdrop-blur-xl shadow-2xl transition-all ${
      isFinished ? "border-emerald-500/40 shadow-emerald-500/10" : "border-white/[0.08]"
    }`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {/* Counter Badges (Dev-Tool Monospace Metrics) */}
        <div className="flex items-center gap-2 sm:gap-3 text-xs font-mono">
          <div className="flex items-center gap-1.5 rounded-xl bg-amber-500/10 border border-amber-500/30 px-3 py-1 text-amber-300 shadow-sm">
            <GitCompare className="h-3.5 w-3.5 text-amber-400" />
            <span className="text-[11px] text-slate-400">Comparisons:</span>
            <strong className="font-bold text-amber-200">{comparisonsCount}</strong>
          </div>

          <div className="flex items-center gap-1.5 rounded-xl bg-rose-500/10 border border-rose-500/30 px-3 py-1 text-rose-300 shadow-sm">
            <ArrowLeftRight className="h-3.5 w-3.5 text-rose-400" />
            <span className="text-[11px] text-slate-400">Swaps:</span>
            <strong className="font-bold text-rose-200">{swapsCount}</strong>
          </div>

          <div className="hidden sm:flex items-center gap-1.5 rounded-xl bg-cyan-500/10 border border-cyan-500/30 px-3 py-1 text-cyan-300 shadow-sm">
      ...
      <div className="rounded-xl border border-slate-700/60 bg-[#070b14]/90 px-4 py-3 min-h-[52px] flex items-center shadow-inner">
        <p className="text-xs sm:text-sm font-medium text-slate-200 leading-relaxed font-mono flex items-center gap-2.5">
          {isFinished ? (
